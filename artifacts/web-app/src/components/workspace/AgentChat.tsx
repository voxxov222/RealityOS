...
export function AgentChat({ projectId, projectName }: AgentChatProps) {
  ...
  const { runAgent, content: streamedContent, isLoading, error, reset } = useAgentStream();
  ...
  const handleSubmit = useCallback(async () => {
    ...
    reset();
  }, [prompt, isLoading, runAgent, projectId, projectName, reset]);
    ...
    <div className="flex flex-col h-[calc(100vh-65px)] w-full relative">
      ...
      <div className="absolute bottom-0 left-0 w-full p-4 bg-gradient-to-t from-background via-background/90 to-transparent pt-12">
        <div className="max-w-4xl mx-auto relative group">
          <div className="absolute -inset-1 bg-gradient-to-r from-primary to-secondary rounded-2xl blur opacity-25 group-focus-within:opacity-50 transition duration-1000 group-hover:duration-200" />
